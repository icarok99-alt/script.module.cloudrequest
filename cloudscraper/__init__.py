...
class CloudScraper(Session):
    ...
    def detect_cloudflare_type(self, response) -> ChallengeType:
        
        return detect_challenge_type(response)

    def get_challenge_info(self, response) -> dict:
        
        return get_challenge_info(response)
    ...
    def decodeBrotli(self, resp):
        if (
            ...
            if self.allow_brotli and resp._content:
                ...
                logger.warning(
                    "urllib3 %s returned Brotli content but allow_brotli=False — skipping.",
    ...
    def request(self, method: str, url: str, *args, **kwargs):
        ...
        try:
            ...
            self.current_concurrent_requests = max(
                0, self.current_concurrent_requests - 1
            )
            raise
        except Exception:
            self.current_concurrent_requests = max(
        ...
        challenge_type = detect_challenge_type(response)

        if self.debug and challenge_type != ChallengeType.NONE:
            info = get_challenge_info(response)
            print(
                f"[CloudScraper] Challenge detected: {challenge_type.name} | "
                f"status={info['status']} | solvable={info['solvable']} | "
                f"CF-RAY={info.get('cf_ray', '')}"
            )

        if challenge_type == ChallengeType.TURNSTILE and not self.disableTurnstile:
            self._solveDepthCnt += 1
            return self.turnstile.handle_Turnstile_Challenge(response, **kwargs)

        if challenge_type == ChallengeType.V3_JS and not self.disableCloudflareV3:
            self._solveDepthCnt += 1
            return self.cloudflare_v3.handle_V3_Challenge(response, **kwargs)

        if challenge_type == ChallengeType.V2_CAPTCHA and not self.disableCloudflareV2:
            self._solveDepthCnt += 1
            return self.cloudflare_v2.handle_V2_Captcha_Challenge(response, **kwargs)

        if challenge_type == ChallengeType.V2_JS and not self.disableCloudflareV2:
            self._solveDepthCnt += 1
        ...
        if challenge_type == ChallengeType.MANAGED:
            logger.warning(
                "Received a Cloudflare Managed challenge at %s that cannot be "
                "solved automatically. Consider adding a captcha provider.",
                url,
            )

        if challenge_type == ChallengeType.UNKNOWN_CF:
            logger.warning(
                "Unrecognised Cloudflare challenge pattern at %s (HTTP %s). "
                "The page may require a newer solver.",
                url,
        ...
        if response.status_code == 403 and self.auto_refresh_on_403:
            if self._403_retry_count < self.max_403_retries:
                ...
                if self.debug:
                    print(
                        f"Received 403 — refreshing session "
    ...
    def _refresh_session(self, url: str) -> bool:
        ...
        try:
            ...
            if self.debug:
                label = "successful" if success else f"failed (HTTP {test_response.status_code})"
                print(f"Session refresh {label}.")
    ...
    def _rotate_tls_cipher_suite(self) -> None:
        ...
        try:
            ...
            available_ciphers: List[str] = (
                browsers_data.get("cipherSuite", {}).get(browser_name, [])
            )
            ...
            start_index = cipher_index % max(1, len(available_ciphers) - num_ciphers + 1)
            new_suite = ":".join(available_ciphers[start_index : start_index + num_ciphers])
            ...
            if self.debug:
                print(
                    f"TLS cipher rotated (#{self._cipher_rotation_count}): "
                    f"{num_ciphers} ciphers from index {start_index}"
                )
    ...
    def create_scraper(cls, sess=None, **kwargs) -> "CloudScraper":
        ...
        if sess:
            for attr in ("auth", "cert", "cookies", "headers", "hooks",
                         "params", "proxies", "data"):
...
if ssl.OPENSSL_VERSION_INFO < (1, 1, 1):
    logger.warning(
        "OpenSSL %s is below the minimum required version (>= 1.1.1) for TLS 1.3. "
